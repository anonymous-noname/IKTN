...
class Base(Module):
    def __init__(self, vocab_size, embedding_size, word_embedding, hidden_size,
        ...
        self.domain_loss = nn.MultiMarginLoss(margin=margin)
    ...
    def predict_eval(self, inputs, matrix, label, set_index):
        predict, mlp_outputs, domain_label, gcn_outputs, reconstruct_feature, event_embed = self.forward(inputs, matrix, label)
...
class GCN(Module):
    ...
    def normalize(a, x=None, in_out=False):
        ...
        a = torch.add(a, torch.eye(a.size(1)).to(device))
...
class lstm(Module):
    def __init__(self, dropout):
        ...
        self.layer = nn.Sequential(
            nn.GRU(512, 512, 2, batch_first=True, bidirectional=False, dropout=dropout),
        )

    def forward(self, inpute):
        context = inpute[:, 0:8, :]
        candidate = inpute[:, 8:13, :]
        (out, h_n) = self.layer(context)
        lstm_out = torch.cat((out,candidate),1)
        return lstm_out
...
class AddAttention(Module):
    ...
    def forward(self, inputs, mask=None):
        ...
        u1 = self.w1(inputs)
        u2 = self.w2(inputs)
        u = torch.zeros(inputs.size(0), arg_len, arg_len).to(device)
...
def compute_mask(positional_size):
    ...
    """
    return torch.triu(torch.fill_(torch.zeros(positional_size, positional_size), -1e9), 1).to(device)
